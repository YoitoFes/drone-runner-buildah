...
import (
	...
	"github.com/YoitoFes/drone-runner-buildah/engine"
	"github.com/YoitoFes/drone-runner-buildah/engine/compiler"
	"github.com/YoitoFes/drone-runner-buildah/engine/linter"
	"github.com/YoitoFes/drone-runner-buildah/engine/resource"
	"github.com/YoitoFes/drone-runner-buildah/internal/match"
...
func (c *daemonCommand) run(*kingpin.ParseContext) error {
	...
	for {
		...
		if err != nil {
			logrus.WithError(err).
				Errorln("cannot ping buildah")
			time.Sleep(time.Second)
		} else {
			logrus.Debugln("successfully pinged buildah")
			break
		}
	}
